...
- 出力は幅1080px・30fpsのMP4（H.264映像／AAC音声）です。ChromeとSafariでの利用を想定しており、動画タブを開いた時点でブラウザのエンコード対応状況を確認します。
- 動画タブを開くと、実際のカード寸法に合わせてH.264/AACの対応状況を確認します。確認中と未対応時はMP4保存を無効化し、未対応理由を表示します。「動画をプレビュー」では指定範囲をローカル音源付きで再生し、終端で自動停止して開始位置へ戻します。
